...
export const createInquiriesService = async (newInquiry) => {
  try {
    const result = await poolRequest()
      ...
      .query(
        "INSERT INTO Inquiries (InquiryId, Email, Description, Status) VALUES (@InquiryId, @Email, @Description, @Status)"
      );
    return result;
  } catch (error) {
    return error;
...
export const updateInquiryService = async (InquiryId, updatedInquiry) => {
  const { Email, Description, Status } = updatedInquiry;
  try {
    const result = await poolRequest()
      .input("InquiryId", sql.VarChar(255), InquiryId)
      .input("Email", sql.VarChar(255), Email)
      ...
      .query(
        "UPDATE Inquiries SET Email=@Email, Description=@Description, Status=@Status WHERE InquiryId=@InquiryId"
      );
    return result;
  } catch (error) {
    return error;
